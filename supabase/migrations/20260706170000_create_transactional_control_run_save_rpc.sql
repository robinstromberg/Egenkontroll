...
comment on function public.save_control_run_transactional(uuid, uuid, uuid, jsonb, jsonb) is
  'Saves a control run, run items, deviations and attachment metadata in one caller-RLS transaction. Storage objects must be uploaded separately before calling this function.';
