export type FactPageContent = {
  ...
  example: { title: string; introduction: string; steps: readonly string[]; note: string };
  mistakes: readonly { title: string; copy: string }[];
  faq: readonly { question: string; answer: string }[];
  source: { label: string; url: string; type: 'myndighetsvägledning'; factCheckedAt: string; limitation: string };
  sequentialNextStep: { href: string; title: string; copy: string };
  appBridge: { title: string; copy: string; href: string; linkLabel: string };
};
...
export const faroanalysFactPage: FactPageContent = {
  ...
  sequentialNextStep: { href: '/seo/kontrollplan.html', title: 'Nästa steg: planera kontroller', copy: 'När farorna är bedömda kan kontrollplanen hjälpa dig att samla kontrollpunkter, ansvar och uppföljning.' },
  appBridge: { title: 'När arbetet återkommer', copy: 'Min Egenkontroll kan stötta återkommande kontroller, avvikelser och historik när verksamheten vill dokumentera sitt löpande arbete.', href: '/digital-egenkontroll-livsmedel', linkLabel: 'Se hur appen fungerar' },
};
