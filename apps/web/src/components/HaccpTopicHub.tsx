...
const practicalResources = ['/mall-kontrollplan-livsmedel', '/verktyg-faroanalys-livsmedel'].map((href) => getPublicResource(href)!);
...
function setMeta() { document.title = content.title; for (const [selector, value] of [['meta[name="description"]', content.description], ['meta[name="robots"]', 'index, follow'], ['meta[property="og:title"]', content.title], ['meta[property="og:description"]', content.description], ['meta[property="og:url"]', 'https://minegenkontroll.se/haccp-sma-livsmedelsforetag']] as const) { const element = document.head.querySelector<HTMLMetaElement>(selector); if (element) element.content = value; } const canonical = document.head.querySelector<HTMLLinkElement>('link[rel="canonical"]'); if (canonical) canonical.href = 'https://minegenkontroll.se/haccp-sma-livsmedelsforetag'; }
...
export function HaccpTopicHub() {
  ...
  return <PublicSiteShell><main className="haccp-shell" id="main-content">
    ...
    <section className="haccp-practical"><div><p className="haccp-kicker">Praktiskt stöd</p><h2>Arbeta vidare med ditt eget underlag</h2><p>Använd mallen och verktyget efter att du har förstått arbetsstegen ovan. Resurserna hjälper dig att strukturera arbetet men ersätter inte verksamhetens egna bedömningar.</p></div><div className="haccp-practical__resources">{practicalResources.map((resource) => <a href={resource.href} key={resource.href}><small>{resource.resourceType}</small><strong>{resource.title}</strong><p>{resource.copy}</p></a>)}</div></section>
    <section className="haccp-flex"><h2>Flexibilitet för små verksamheter</h2><p>{content.practicalText}</p></section><section className="haccp-faq"><h2>Frågor och svar</h2>{content.faq.map(([question, answer]) => <article key={question}><h3>{question}</h3><p>{answer}</p></article>)}</section>
    <section className="haccp-source"><h2>Källa och begränsning</h2><p><strong>Källtyp:</strong> myndighetsvägledning.</p><p><a href={content.sourceUrl}>{content.sourceLabel}</a></p><p>{content.sourceNote}</p></section>
    <section className="haccp-app"><p className="haccp-kicker">När arbetet återkommer</p><h2>Dokumentera löpande arbete i appen.</h2><p>Min Egenkontroll kan stötta återkommande kontroller, avvikelser och historik när rutinerna ska följas upp.</p><LinkButton href="/digital-egenkontroll-livsmedel" variant="primary">Se hur appen fungerar</LinkButton></section><section className="haccp-related"><h2>Relaterade ämnen</h2><a href="/kunskapsbank">Utforska kunskapsbanken</a><a href="/sparbarhet-livsmedel">Spårbarhet</a></section>
