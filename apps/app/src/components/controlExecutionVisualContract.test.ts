...
test('control forms retain required, disabled, offline and save ordering', async () => {
  ...
  assert.match(photoForm, /if \(!definition \|\| missingAction \|\| incompleteColdStorage\) return;/);
  assert.match(photoForm, /disabled=\{saving \|\| definition\.fields\.length === 0 \|\| missingAction \|\| incompleteColdStorage \|\| !isOnline\}/);
  assert.match(legacyForm, /has_photo:\s*false/);
  assertOrdered(photoForm, [
    'const savedRun = await saveControlRun',
    "eventName: 'control_saved'",
    'savedRun.responseOutcomes?.length',
  ...
  assertOrdered(photoForm, [
    'readColdStorageDraft(window.localStorage',
    'writeColdStorageDraft(window.localStorage',
    'await saveControlRun',
    'clearColdStorageDraft(window.localStorage',
  ]);
